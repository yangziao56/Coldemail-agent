...
- 最初版本，通过 JSON 输入 Sender/Receiver/Goal，直接生成冷邮件。

### v1.0 - PDF 简历解析

- 支持上传 PDF 简历，自动解析并提取 SenderProfile。

### v1.1 - 网络搜索获取 receiver 信息

- Web 搜索抓取 receiver 信息，用户只需提供名字和领域。
- 支持 DuckDuckGo/Bing 搜索，自动解析网页内容并提炼结构化资料。

### v1.2 - 切换到 Google Gemini API

- API 迁移：从 OpenAI GPT-4o-mini 切换到 Google Gemini（默认 `gemini-2.0-flash`）。
- 环境变量改为 `GEMINI_API_KEY`/`GOOGLE_API_KEY`，依赖更新为 `google-generativeai`。
